...
parser.add_argument('--cachePath', type=str, default='/home', help='Path to save cache to.')
...
def test(eval_set, epoch=0, write_tboard=False):
    ...
    with torch.no_grad():
        ...
        for iteration, (input, indices) in enumerate(test_data_loader, 1):
            ...
            #       "netvlad_encoder.onnx",   # where to save the model (can be a file or file-like object)
            ...
            vlad_encoding = model.pool(image_encoding)
            # torch.onnx.export(model.pool,               # model being run
            #       image_encoding,                         # model input (or a tuple for multiple inputs)
            #       "netvlad_pool.onnx",   # where to save the model (can be a file or file-like object)
            #       export_params=True,        # store the trained parameter weights inside the model file
    ...
    if not(only_db):
        ...
        shift = 0
    for num_q, prediction in enumerate(predictions):
        # output[eval_set.images[eval_set.dbStruct.numDb+i-1]] = {}
        # output[eval_set.images[eval_set.dbStruct.numDb+i-1]]['database_images'] = list([eval_set.images[j] for j in prediction])
        # output[eval_set.images[eval_set.dbStruct.numDb+i-1]]['cos_metric'] = []
        for j, num_db in enumerate(prediction):
            dbfeature = dbFeat[num_db]
            qfeature = qFeat[num_q]
...
if __name__ == "__main__":
    ...
    if opt.mode.lower() == 'train':
        ...
        whole_test_set = dataset.get_whole_test_set(opt.metadata_json_file, onlyDB=False)
