...
def main(args):
    sequence = []
    with open(os.path.join(args.seq, 'rgb_left.txt'), 'r') as f_left, open(os.path.join(args.seq, 'depth.txt'), 'r') as f_depth:
        for line_left, line_depth in zip(f_left, f_depth):
            ...
            sequence.append((float(ts), os.path.join(args.seq, img_left_path), os.path.join(args.seq, depth_path)))
